const API = {
  auth:    "http://localhost:5105",
  client:  "http://localhost:5101",
  produs:  "http://localhost:5102",
  factura: "http://localhost:5103",
  plata:   "http://localhost:5104",
};
...
createApp({
  ...
  methods: {
    ...
    async resetParola() {
      const r = await fetch(`${API.auth}/Auth/ResetPassword`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
    ...
    conecteazaLive() {
      const conn = new signalR.HubConnectionBuilder()
        .withUrl(`${API.client}/clientHub`, { withCredentials: false })
        .build();
    ...
    async incarcaProduse() {
      const r = await fetch(`${API.produs}/Produs/GetProduse`, { headers: this.headere() });
      this.produse = await r.json();
    },
    async adaugaProdus() {
      await fetch(`${API.produs}/Produs/PostProdus`, {
        method: "POST",
    ...
    async incarcaClienti() {
      const r = await fetch(`${API.client}/Client/GetClienti`, { headers: this.headere() });
      this.clienti = await r.json();
    },
    async adaugaClient() {
      await fetch(`${API.client}/Client/PostClient`, {
        method: "POST",
        headers: this.headere(),
    ...
    async stergeClient(id) {
      await fetch(`${API.client}/Client/DeleteClient?id=${id}`, {
        method: "DELETE",
        headers: this.headere(),
      });
    ...
    async creeazaFactura() {
      await fetch(`${API.factura}/Factura/AdaugaFactura`, {
        method: "POST",
        headers: this.headere(),
    ...
    async incarcaFacturi() {
      const r = await fetch(`${API.factura}/Factura/GetFacturi`, { headers: this.headere() });
      this.facturi = await r.json();
    },
    async incarcaFacturiClient() {
      if (this.filtruClient === 0) return;
      const r = await fetch(`${API.factura}/Factura/GetFacturiByClient?idClient=${this.filtruClient}`, { headers: this.headere() });
    ...
    async incarcaPlati() {
      const r = await fetch(`${API.plata}/Plata/GetPlati`, { headers: this.headere() });
      this.plati = await r.json();
    },
    async adaugaPlata() {
      const r = await fetch(`${API.plata}/Plata/PostPlata`, {
        method: "POST",
        headers: this.headere(),
